    ...
    const presentations: SemesterPresentations[] = [
        {
            ...
            items: [
                ...
                { title: "Neural Networks Basics", link: "/presentations/w25/nn_basics.pdf" }
            ]
        },
        {
            semester: "Fall 2024",
                            ...
                            {semester.items.length > 0 ? (
                                <div className="space-y-4">
                                    {semester.items.map((item, itemIdx) => (
                                        <div key={itemIdx} className="bg-gray-100 p-4 rounded-lg flex justify-between items-center hover:bg-gray-200 transition-colors">
                                            <span className="font-semibold">{item.title}</span>
                                            <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-seth-coral font-bold hover:underline">
                                                PDF
                                            </a>
                                        </div>
                                    ))}
